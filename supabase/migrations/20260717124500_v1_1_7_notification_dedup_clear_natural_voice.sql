-- DraBornGarage v1.1.7
-- Keep one active push token per user/platform, accept the natural Turkish voice,
-- and move Turkish Android channels to immutable v10 IDs.
begin;

-- Existing test installs created a new installation identity more than once.
-- Keep only the most recently seen token for each user/platform immediately.
with ranked as (
  select
    id,
    row_number() over (
...
update public.notification_push_tokens token
set enabled=false, updated_at=now()
from ranked
where token.id=ranked.id and ranked.row_number>1;
...
begin
  ...
  v_platform:=case when p_platform in ('android','ios') then p_platform else 'unknown' end;

  -- DraBornGarage currently keeps one active installation per account/platform.
  -- This prevents old APK installations from receiving the same event repeatedly.
  ...
  update public.notification_push_tokens
  set enabled=false, updated_at=now()
  where user_id=v_user and platform=v_platform and id<>v_id and enabled;
  ...
  return jsonb_build_object(
    'id',v_id,
    'registered',true,
    'single_active_platform_token',true,
    'retried_recent_notifications',true
  );
end;
$$;
...
begin
  ...
  if not found
    ...
    or v_notification.push_sent_at is not null
  then
    return 0;
  end if;

  select coalesce(preference.notification_sound,'garage_chime'),
         coalesce(preference.push_notifications_enabled,true)
  into v_sound,v_push_enabled
  from public.notification_preferences preference
  where preference.user_id=v_notification.user_id;
  ...
  -- Defensive selection: even if stale rows are manually re-enabled, only the
  -- newest token of each platform receives this notification.
  for v_token in
    select distinct on (platform) id,expo_push_token,platform
    from public.notification_push_tokens
    where user_id=v_notification.user_id and enabled
    order by platform,last_seen_at desc,updated_at desc,created_at desc,id desc
  loop
  ...
  set push_attempted_at=now(),
      push_error=case
        when v_count>0 then 'Expo teslim yanıtı bekleniyor'
        else 'Kayıtlı Expo push cihazı bulunamadı'
      end
  where id=p_notification_id;
  return v_count;
...
as $$
select case
when p_sound='turkish_voice' and p_category='appointments' then 'draborngarage-voice-appointment-v10'
when p_sound='turkish_voice' and p_category='customer_links' then 'draborngarage-voice-customer-link-v10'
when p_sound='turkish_voice' and p_category='service' then 'draborngarage-voice-service-v10'
when p_sound='turkish_voice' and p_category in ('payments','receivables','platform') then 'draborngarage-voice-payment-v10'
when p_sound='turkish_voice' then 'draborngarage-voice-generic-v10'
