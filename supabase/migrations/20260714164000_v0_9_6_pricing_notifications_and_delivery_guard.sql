create or replace function public.notification_delete(p_notification_id uuid)
returns boolean
language plpgsql
security definer
...
grant execute on function public.notification_delete(uuid) to authenticated;
...
begin
  ...
  if new.status = 'delivered'::public.work_order_status
    ...
    raise exception 'Motoru teslim etmeden önce son net ücret veya işlem tutarı kaydedilmelidir.';
  end if;
...
begin
  ...
  if p_status='delivered' and coalesce(target_total,0)<=0 then
    raise exception 'Motoru teslim etmeden önce son net ücret veya işlem tutarı kaydedilmelidir.';
  end if;
...
revoke all on function public.notification_get_center(integer) from anon;
