...
- Admin Paneli ile İşletme ve Ekip ekranında işletme oluşturma/düzenleme desteği.

## Güvenlik kararı
Plaka tek başına müşteri hesabını otomatik açmaz. Son bağlantı Usta/İşletme yetkisiyle onaylanır. RPC'ler işletme sahibi veya aktif Usta rolü kontrolü yapar. Yeni RPC'lerde anonim çalıştırma yetkisi yoktur.

## Doğrulama durumu
- Supabase migration uygulandı: `20260711191813_v0_8_3_customer_motor_tax_linking`.
- Yeni profil ve işletme sütunları, plaka indeksi ve RPC yetkileri doğrulandı.
- Yetkili Usta ile plaka arama ve doğrudan eşleştirme akışı transaction içinde uçtan uca çalıştırıldı; test verileri `ROLLBACK` ile geri alındı.
...
## Sonraki adım
`v0.8.3` tamamlandıktan sonra `v0.9.0 — Google Play Uyum, Test ve Pilot` aşamasına geçilir.
