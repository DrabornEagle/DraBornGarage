...
export function CustomerMotorcyclesScreen({ onStartLink }: { onStartLink: () => void }) {
  ...
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={async () => { setRefreshing(true); await load(); setRefreshing(false); }} tintColor={colors.primary} />}>
    ...
    {items.length === 0 ? <GlassCard style={styles.empty}><AnimatedMotorcycleIcon size={46} color={colors.textMuted} active={false} /><Text style={[styles.emptyTitle, { color: colors.text }]}>Motor bulunamadı</Text></GlassCard> : items.map((item) => <GlassCard key={item.id} style={styles.card}>
      <View style={styles.top}><View style={[styles.icon, { backgroundColor: `${colors.primary2}18` }]}><AnimatedMotorcycleIcon size={34} color={colors.primary2} /></View><View style={styles.copy}><Text style={[styles.title, { color: colors.text }]}>{item.brand} {item.model}</Text><Text style={[styles.meta, { color: colors.textMuted }]}>{item.plate || 'Plaka yok'}{item.year ? ` • ${item.year}` : ''}{item.color ? ` • ${item.color}` : ''}</Text></View>{item.latest_status && <StatusPill status={item.latest_status} />}</View>
...
const styles = StyleSheet.create({
  content: { paddingHorizontal: 18, paddingTop: 56, paddingBottom: 32, gap: 14 }, card: { gap: 13 }, top: { flexDirection: 'row', alignItems: 'center', gap: 10 }, icon: { width: 53, height: 53, borderRadius: 18, alignItems: 'center', justifyContent: 'center' }, copy: { flex: 1, minWidth: 0 }, title: { fontSize: 17, fontWeight: '900' }, meta: { fontSize: 12.5, marginTop: 4 }, stats: { flexDirection: 'row', gap: 8 }, stat: { flex: 1, minHeight: 70, borderRadius: 16, padding: 10, justifyContent: 'center' }, statValue: { fontSize: 17, fontWeight: '900' }, statLabel: { fontSize: 11, marginTop: 4 }, footer: { flexDirection: 'row', alignItems: 'center', gap: 7 }, footerText: { fontSize: 12.5 }, empty: { alignItems: 'center', gap: 9, paddingVertical: 30 }, emptyTitle: { fontSize: 17, fontWeight: '900' },
});
