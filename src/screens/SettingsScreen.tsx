...
export function SettingsScreen() {
  ...
  return <ScrollView contentContainerStyle={styles.content}>
    ...
    <SettingsAccordion title="Uygulama" subtitle="v1.0.1 RC • Expo düzeltme testleri" icon="information-circle" accent={colors.green} open={openSection === 'app'} onToggle={() => toggleSection('app')}>
      <GlassCard style={styles.info}><Info icon="layers" label="Sürüm" value="v1.0.1 RC • Expo Test Adayı" /><Info icon="shield-checkmark" label="Motor Hazır kuralı" value="Son net ücret girilmeden Motor Hazır yapılamaz" /><Info icon="key" label="Şifre güvenliği" value="En az 6 karakter • basit kayıt şifresi" /><Info icon="archive" label="Bu sürüm öncesi yedek" value="backup/v1.0.0-rc-before-v1.0.1-20260715" /><Info icon="refresh" label="Geri alma" value="Kod yedeğiyle v1.0.0 RC" /><Info icon="phone-portrait" label="Test yöntemi" value="Expo Go + GitHub kalite testleri" /><Info icon="storefront" label="Mağaza durumu" value="Expo testinde • sonraki adım Test APK" /></GlassCard>
    </SettingsAccordion>
