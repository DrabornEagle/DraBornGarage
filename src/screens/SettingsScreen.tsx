...
export function SettingsScreen() {
  ...
  return <ScrollView contentContainerStyle={styles.content}>
    ...
    <SettingsAccordion title="Uygulama" subtitle={`${APP_VERSION_LABEL} • Google Play Yayın Sürümü`} icon="information-circle" accent={colors.green} open={openSection === 'app'} onToggle={() => toggleSection('app')}>
      <GlassCard style={styles.info}><Info icon="layers" label="Sürüm" value={`${APP_VERSION_LABEL} • Google Play Yayın Sürümü`} /><Info icon="shield-checkmark" label="Motor Hazır kuralı" value="Ücret isteğe bağlı • Tahsilat veya borç tutarı Net Fiyat olabilir" /><Info icon="key" label="İmza güvenliği" value="Kalıcı DraBornGarage production upload keystore" /><Info icon="archive" label="Bu sürüm öncesi yedek" value="backup/v1.1.8final" /><Info icon="refresh" label="Geri alma" value="Kod ve veritabanıyla v1.1.8final" /><Info icon="phone-portrait" label="Test yöntemi" value="Release APK + Google Play AAB" /><Info icon="storefront" label="Mağaza durumu" value="v1.0 / versionCode 1 üretim ve son cihaz testleri" /></GlassCard>
    </SettingsAccordion>
